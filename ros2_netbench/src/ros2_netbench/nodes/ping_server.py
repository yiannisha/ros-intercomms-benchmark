"""Ping/pong RTT benchmark server."""
...
class PingServer(Node):
    def __init__(self, config: RunConfig) -> None:
        super().__init__("netbench_ping_server")
        self.config = config
        qos = qos_profile(config.qos)
        self.publisher = self.create_publisher(BenchmarkPacket, config.echo_topic, qos)
        self.subscription = self.create_subscription(BenchmarkPacket, config.topic, self._on_ping, qos)
    ...
    def _on_ping(self, packet: BenchmarkPacket) -> None:
        now = monotonic_ns()
        ...
        summary.update(
            {
                ...
                "RTT": {"valid": False, "reason": "computed_by_ping_client"},
                "one_way_latency": {"valid": False, "reason": "not_applicable_for_ping_server"},
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--mode" not in args_list:
        args_list = ["--mode", "ping", "--role", "server", *args_list]
