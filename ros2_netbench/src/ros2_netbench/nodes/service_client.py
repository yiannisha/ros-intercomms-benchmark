"""ROS 2 service latency benchmark client."""
...
class ServiceClient(Node):
    def __init__(self, config: RunConfig) -> None:
        super().__init__("netbench_service_client")
        self.config = config
    ...
    def wait_for_service_ready(self, timeout_s: float) -> float | None:
        start = monotonic_ns()
        while rclpy.ok():
    ...
    def _on_timer(self) -> None:
        ...
        if now < self.measure_end_ns:
            ...
            request.packet = self._make_packet(seq, now, send_wall)
            self.pending[seq] = PendingRequest(seq, now, send_wall, warmup=warmup)
            future = self.client.call_async(request)
            future.add_done_callback(lambda fut, seq=seq: self._on_response(seq, fut))
        elif now > self.measure_end_ns + int(self.config.request_timeout_s * 1_000_000_000):
            self.finished = True
            if self.timer is not None:
                self.timer.cancel()

    def _make_packet(self, seq: int, send_mono_ns: int, send_wall_ns: int) -> BenchmarkPacket:
        packet = BenchmarkPacket()
        packet.seq = seq
        packet.sender_monotonic_ns = send_mono_ns
    ...
    def _on_response(self, seq: int, future: Any) -> None:
        now = monotonic_ns()
        ...
        event = self.tracker.observe(seq)
        ...
        self.rows.append(
            {
                ...
                "rtt_ns": rtt,
                "payload_size": response.packet.payload_size,
...
def run(config: RunConfig) -> int:
    ...
    try:
        ...
        discovery_time_ms = node.wait_for_service_ready(config.discovery_timeout_s)
        node.start()
        last_sample = 0
        ...
        rtt["reason"] = "ok" if node.rtt_ns else "no_service_latency_samples"
        summary.update(
            {
                ...
                "RTT": rtt,
                "service_latency": rtt,
                "one_way_latency": empty_latency_summary(False, "not_applicable_for_service"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--mode" not in args_list:
        args_list = ["--mode", "service", "--role", "client", *args_list]
